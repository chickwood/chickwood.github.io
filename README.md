...
纯静态站点，支持：
- 项目菜单（当前配了两个占位项目，可以加更多）
- 每个项目独立的 release 发布页
- 中(简)/日/英三语界面，根据浏览器语言自动切换，也可手动切换并记住选择
- release 数据由 **GitHub Actions 定时任务**抓取并写成静态 JSON，页面本身不再直接调用 GitHub API

## 1. 配置项目

打开 `config/projects.json`，把两个占位项目改成你自己的：

```json
{
  "key": "project-a",          // 内部标识，随便起，跟 data/ 里的文件名对应
  "owner": "your-username",    // GitHub 用户名/组织名
  "repo": "your-repo-a",       // 仓库名
...
**这次不能直接双击打开 `index.html` 了** —— 因为页面现在读取的是本地的 `data/*.json` 文件，浏览器出于安全限制，不允许 `file://` 页面用 `fetch` 读取本地文件（会报 CORS 或跨域错误）。必须起一个本地服务器：
...
`data/` 目录里已经放了示例数据，本地打开就能直接看到效果，正式跑起定时任务后会被真实数据覆盖。

## 3. 让项目发布 release 时自动触发更新

`.github/workflows/update-releases.yml` 现在有两种触发方式：

- **手动触发**：Actions 标签页点 "Run workflow"
- **被动触发**：收到 `repository_dispatch` 事件（`release-published`）时自动跑

定时触发（`schedule`）默认已经注释掉，想恢复的话把那两行取消注释即可。
...
3. 推送后，Action 会按 `.github/workflows/update-releases.yml` 里配置的时间自动跑（默认每 6 小时一次），也可以去 **Actions** 标签页手动点 **Run workflow** 立即触发一次
...
- **release 的更新说明正文不做翻译**，原样显示你在 GitHub Release 里写的内容
...
├── scripts/fetch-releases.mjs      # 定时任务实际跑的抓取脚本
├── templates/notify-release-site.yml   # 复制到各项目仓库，让它们发布 release 时通知这边更新
└── .github/workflows/update-releases.yml   # 更新数据的 workflow（手动 / 被通知 / 可选定时）
```
